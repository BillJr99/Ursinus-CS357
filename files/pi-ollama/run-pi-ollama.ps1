...
.DESCRIPTION
  The PowerShell twin of run-pi-ollama.sh, for Windows students who are not
  using WSL. It builds the same container, runs the same launcher, and talks to
...
.PARAMETER Mode
  One of Recover, Continue, or Resume. Omit for a new session.
    Recover  - start fresh from the RESUME.md checkpoint
    Continue - reopen the last session
    Resume   - pick from saved sessions

.EXAMPLE
  .\run-pi-ollama.ps1
.EXAMPLE
  .\run-pi-ollama.ps1 -Mode Recover
#>
[CmdletBinding()]
param(
    [ValidateSet('Recover', 'Continue', 'Resume')]
    [string]$Mode
)
...
function Get-Setting {
    # PowerShell has no ${VAR:-default}, so this is that idiom as a function:
    # use the environment variable if the user set one, otherwise the default.
...
$PiMaxContext      = Get-Setting 'PI_MAX_CONTEXT'        '0'      # 0: no extra client ceiling
$PiMaxOutputTokens = Get-Setting 'PI_MAX_OUTPUT_TOKENS'  '8192'
$PiKeepRecent      = Get-Setting 'PI_KEEP_RECENT_TOKENS' '4096'
$PiReasoning       = Get-Setting 'PI_REASONING'          'auto'   # auto, on, off
$PiThinkingLevel   = Get-Setting 'PI_THINKING_LEVEL'     'medium' # off, low, medium, high
$PiRequestTimeout  = Get-Setting 'PI_REQUEST_TIMEOUT'    '300'

# Windows has no POSIX uid, so user mode inside the container needs a numeric id
# to drop to. 1000 is the conventional first non-root user on Debian images.
$PiUserUid = Get-Setting 'PI_USER_UID' '1000'
$PiUserGid = Get-Setting 'PI_USER_GID' '1000'

if ($PiRunAs -notin @('root', 'user')) {
    throw "PI_RUN_AS must be root or user."
}
if ($PiUserUid -notmatch '^\d+$' -or $PiUserGid -notmatch '^\d+$') {
    throw "PI_USER_UID and PI_USER_GID must be numeric."
}
if ($PiRunAs -eq 'user' -and [int]$PiUserUid -eq 0) {
    throw "User mode requires a nonzero PI_USER_UID; set the intended UID/GID."
}

$LaunchMode = switch ($Mode) {
    'Recover'  { 'recover' }
    'Continue' { 'continue' }
    'Resume'   { 'resume' }
    default    { 'new' }
...
Write-Host "[launcher] Project RW mount: $ProjectDir"
...
# is identical to the CONTAINER_SCRIPT heredoc in run-pi-ollama.sh. The @' '@
# here-string is single-quoted, so PowerShell does no interpolation: every $ and
# ` below reaches bash exactly as written.
$ContainerScript = @'
set -euo pipefail
...
# Windows line endings would reach bash as literal carriage returns and break
# every line of the script with "command not found". Normalize before sending.
...
$dockerArgs = @(
    ...
    '--workdir', '/workspace',
    '-e', "PI_PACKAGE=$PiPackage",
    '-e', "PI_RUN_AS=$PiRunAs",
    '-e', "PI_USER_UID=$PiUserUid",
    '-e', "PI_USER_GID=$PiUserGid",
    '-e', "PI_OLLAMA_URL=$PiOllamaUrl",
    '-e', "PI_FALLBACK_CONTEXT=$PiFallbackContext",
    '-e', "PI_MAX_CONTEXT=$PiMaxContext",
    '-e', "PI_MAX_OUTPUT_TOKENS=$PiMaxOutputTokens",
    '-e', "PI_KEEP_RECENT_TOKENS=$PiKeepRecent",
    '-e', "PI_REASONING=$PiReasoning",
    '-e', "PI_THINKING_LEVEL=$PiThinkingLevel",
    '-e', "PI_REQUEST_TIMEOUT=$PiRequestTimeout",
    '-e', "PI_OLLAMA_MODEL=$PiOllamaModel",
    '-e', "PI_LAUNCH_MODE=$LaunchMode",
    $BaseImage, 'bash', '-lc', $ContainerScript
)
