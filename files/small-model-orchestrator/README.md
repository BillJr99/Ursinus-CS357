...
with equivalent host tools. Git helpers are optional and require Git only when used.
...
Reserve headroom for output, reasoning where applicable, and tool-result growth.
...
`RESUME.md` holds the active checkpoint; contract, plan, evidence, failures, and
artifacts retain supporting detail. Preserve existing task state during updates.
...
`scripts/checkpoint.py` accepts a JSON object on standard input, validates its
...
| Connection lost, timeout, or process crash | Treat in-flight effects as UNKNOWN and verify before replay. |
...
2. Read RESUME.md in bounded form, confirm task identity and current intent, and
   verify relevant disk state and any in-flight effects.
