...
info:
  ...
  - weight: 35
    ...
    beginning: Some process artifacts exist but are incomplete, the decision log has gaps, or signatures are missing from a progress report
    progressing: The Team Playbook is followed with minor lapses, standups, decision log, meeting notes, role rotation, and all-member signatures are present, but one element is thin or late (Goal 13)
    proficient: The Team Playbook is followed consistently, standup notes and the decision log are current, POGIL roles rotate on schedule, every progress report carries every member's signature, each team document names a primary author per section, and each team document's version or commit history shows a real drafting trajectory rather than a single late paste (Goal 13)
  - weight: 30
...
| 16 | **Sprints 1-3** | Three sprints between the proposal and Demo Day, with rotating roles, runnable increments, evaluation updates, and Responsible AI measurements (token counts, traces, correctness audits, and bias probes) logged at every boundary. Sprint 2 spans the Thanksgiving break; plan for it in your charter's cadence. | Track pages |
...
| 19 | **Demo Day** | Technical demo, a non-technical stakeholder-facing segment, a disseminable artifact, and the Responsible AI Report on the team's own work, the team's deliberate adjourning: final reflection, contribution statements, and your professional portfolio artifact close the thread. | Track pages |
...
| Professionalism and Process (applied at every thread milestone; points are awarded on the individual milestone pages, not here) (35%) | There is no evidence of team process, no meeting notes, no decision log, no role rotation, and missing signatures | Some process artifacts exist but are incomplete, the decision log has gaps, or signatures are missing from a progress report | The Team Playbook is followed with minor lapses, standups, decision log, meeting notes, role rotation, and all-member signatures are present, but one element is thin or late (Goal 13) | The Team Playbook is followed consistently, standup notes and the decision log are current, POGIL roles rotate on schedule, every progress report carries every member's signature, each team document names a primary author per section, and each team document's version or commit history shows a real drafting trajectory rather than a single late paste (Goal 13) |
