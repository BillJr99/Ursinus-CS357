...
The npm equivalent swaps in `npm ci && npm publish` with an `NPM_TOKEN` stored as a repository secret.  Notice what triggers the workflow: a *tag you push deliberately* (not every commit) so the human decision to publish survives the automation.  That design is the AI-maker module's CI lesson and the human publication gate from the *Governance, Policy, and the Cost of Inference* session, implemented in seven lines of YAML.
