...
2.  **Version control, authenticated with an SSH key.**  You will push to GitHub every week this semester, so set authentication up once, now, with a key.  GitHub no longer accepts your account password over HTTPS, and a key is the option that keeps working without a prompt on every push.
    ...
    *On the container route*, the key lives on your host and the container needs to see it; Step 6 of the [Development Environment activity]({{ site.lia_viewer_url }}{{ site.raw_pages_url }}Activities/liascript-devenvironment.md) shows the read-only `~/.ssh` mount that does this, and explains what you are handing the container when you do.
