...
**Before the session,** post a brief [reading response]({{ site.baseurl }}/Participation/ReadingResponses) to the presenter's source: one takeaway, one question, and one connection to something we have built or run.  When the source is circulated the same day, arrive with one real question ready instead.

**During the session,** engage.  Build on the presenter's framing, offer a counter-view, or connect their source to a system we have worked with.

The [Reading Responses]({{ site.baseurl }}/Participation/ReadingResponses) guide explains the format, and audience engagement is assessed within the Class Activities and Participation component described on the [syllabus]({{ site.baseurl }}/).  This is what makes the student-led sessions worth holding, and it means everyone, not only the presenter, has a stake in them.
